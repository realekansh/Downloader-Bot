...
from utils.downloader import DownloaderError, UnsupportedPlatformError, get_video_info
...
logger = logging.getLogger('hypertech.download')
...
async def cmd_download(message: Message):
    ...
    await handle_download(message, urls[0] if urls else None)


@router.message(F.text.regexp(URL_PATTERN) | F.caption.regexp(URL_PATTERN))
async def auto_download(message: Message):
    ...
    urls = URL_PATTERN.findall(text)
    if urls:
        await handle_download(message, urls[0], is_auto=True)


async def handle_download(message: Message, url: str | None, is_auto: bool = False):
    """Core download logic."""
    if not url:
    ...
    # Guard against missing from_user
    if not message.from_user:
        return
    ...
    with get_db() as db:
        ...
        # Run blocking yt-dlp extraction in a thread to avoid freezing the event loop
        try:
            ...
            logger.exception("Unexpected error during info extraction for %s", url)
            await status_msg.edit_text(
                panel(
                    'Error',
                    [detail_text('Reason', 'An unexpected error occurred while checking the link.')],
        ...
        if settings.DEV_MODE:
            ...
            try:
                ...
                logger.exception("DEV_MODE download failed for download_id=%s", download_id)
            return
